...
export const distance2D = (first: Point2D, second: Point2D) => Math.sqrt((first.x - second.x) ** 2 + (first.y - second.y) ** 2);
...
export const intervalIntersection = (
  ...
  const intersection = clamp(right, clampLeft, clampRight) - clamp(left, clampLeft, clampRight);
